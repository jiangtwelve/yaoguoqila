---
status: active
current_task: TASK-011
current_release: v0.1
updated: 2026-06-10
---

# Tasks

## Current
- TASK-011: 搭建微信云开发后端基础能力。`release: v0.1`，已补充 UI 基线验收、保存按钮状态微调、新增临期确认运行时修复、保存点击无反馈修复、确认弹窗文案超限修复、风险确认文案优化、统一 `GlassModal` 弹窗组件、备注 `textarea` 触摸冲突修复、项目级组件抽取规则、表单 loading/saving 锁定逻辑、昵称/创建家庭弹窗极简输入焦点、临期天数高亮、保存全屏 loading、自动真机调试 `subPackages` 兼容配置和本地 agent/tooling 忽略规则；继续在干净数据下进行微信开发者工具真实联调。

## Ready
- TASK-012: 家庭切换功能。`release: v0.1`
- TASK-013: 图片云存储 adapter 正式接入。`release: v0.1`
- TASK-014: 加入家庭与邀请最小流程。`release: v0.1`
- TASK-015: v0.1 测试版验收与体验修复。`release: v0.1`

## Blocked
- 暂无。

## Backlog
- 首页大数据量列表策略设计：近期优先、分组折叠/懒加载，必要时配合游标分页和虚拟列表。`deferred`
- 家庭成员邀请与权限细分。`v0.3 candidate`
- 微信订阅消息正式模板配置。
- Android/iOS 推送服务接入。
- 条码识别。
- OCR 识别日期和保质期。
- AI 菜谱或消耗建议。
- 多语言。

## Done
- TASK-001: 完成产品发现与技术方向确认。
- TASK-002: 确认 MVP 页面地图与首个 Design Anchor。
- TASK-003: 搭建跨端前端项目骨架与 mock service 结构。
- TASK-004: 重新确认家庭首页 Design Anchor。
- TASK-005: 新增/编辑物品表单原型验收完成。
- TASK-006: 物品详情与处理操作原型验收完成。
- TASK-007: 多图上传与首页封面图验收完成。
- TASK-008: 首页物品左滑删除与二次确认验收完成。
- TASK-009: 全页面下拉刷新验收完成。
- TASK-010: API contract 验证完成。
